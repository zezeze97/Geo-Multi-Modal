...
class BunnyMetaModel:
    ...
    def initialize_vision_modules(self, model_args):
        ...
        pretrained_vision_tower_pretrained_local_path = model_args.vision_tower_pretrained_local_path
        ...
        if self.get_vision_tower() is None:
            ...
            vision_tower.load_model(ckpt_path=pretrained_vision_tower_pretrained_local_path)
        ...
        if pretrain_mm_mlp_adapter is not None:
            print(f'load mm mlp adapter from {pretrain_mm_mlp_adapter}')
            mm_projector_weights = torch.load(pretrain_mm_mlp_adapter, map_location='cpu')

            def get_w(weights, keyword):
                return {k.split(keyword + '.')[1]: v for k, v in weights.items() if keyword in k}
            state_dict = get_w(mm_projector_weights, 'mm_projector')
            self.mm_projector.load_state_dict(state_dict)
            # print(f'mm_projector loaded!')
...
class BunnyMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        # print(f'image_features shape is {image_features.shape}') (1, 729, 1024)
