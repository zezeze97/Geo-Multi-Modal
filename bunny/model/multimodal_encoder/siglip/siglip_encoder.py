...
class SigLipVisionTower(nn.Module):
    def __init__(self, vision_tower, vision_tower_cfg, delay_load=False):
        ...
        if not delay_load:
            self.load_model(ckpt_path=vision_tower_cfg.vision_tower_pretrained_local_path)
        else:
            self.cfg_only = self.config

    def load_model(self, ckpt_path=None):
        if self.is_loaded:
            print(f'siglip vision tower is loaded, skip....')
            return
        if ckpt_path is None:
            print(f'loading siglip from {self.vision_tower_name}')
            self.vision_tower = SigLipVisionModel.from_pretrained(self.vision_tower_name)
        else:
            print(f'loading siglip from {ckpt_path}')
            # loading
            self.vision_tower = SigLipVisionModel.from_pretrained(ckpt_path)

        del self.vision_tower.vision_model.encoder.layers[-1:]
        self.vision_tower.vision_model.head = nn.Identity()
        # self.vision_tower.requires_grad_(False) # make vision tower trainable!!!
        # self.vision_tower.eval() # make vision tower trainable!!!

        self.is_loaded = True

    # @torch.no_grad() # make vision tower trainable!!!
